...
int main() {
	...
	if (MessageBoxW(NULL, L"Peaceful version doesn't harm your computer\r\n\Run?", L"Magnitude", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"EPILEPSY WARNING!!!", L"F̷i̷n̸a̴l̴ ̴W̸a̴r̸n̷i̸n̷g̷  - Magnitude", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
